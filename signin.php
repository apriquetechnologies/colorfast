<?php
	...
	if(!empty($_POST)){
		...
		$error_msg = '';
		$signin_result = $lumise->connector->signin();
		if($signin_result){
	...
	if($lumise->connector->get_session('user_id')){		header('location:/');	}
	if(isset($_GET['logout']) && $_GET['logout'] == 'true'){
		$lumise->connector->set_session('user_id', null);
		$lumise->connector->set_session('user_email', null);
		$lumise->connector->set_session('user_first_name', null);
		header('location:/');
	}
?>
<!DOCTYPE html>
...
<body>
	...
	<div id="login-form">
		...
		<h1>
			<?php echo $lumise->lang('Login to your account'); ?>
		</h1>
		<form method="post" action="" id="main-form"<?php if ($action == 'reset'){echo ' style="display: none"';} ?>>
			<?php						
				...
				?>
				<?= $error_msg != '' ? '<p class="login-msg msg-error" style="text-align: center;">'.$error_msg.'</p>':''; ?>
				<div class="inputContainer">
